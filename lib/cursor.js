// Shared lexer primitives used by both the envelope (RFC 5321) and the
// header (RFC 5322) grammars.
//
// All regexes use the sticky (`y`) flag so the cursor can advance in
// O(1) without slicing the input string. They are anchored to the
// cursor position via `lastIndex`. None use backreferences or nested
// quantifiers — bounded matching only — so they are not vulnerable to
// ReDoS.
//
// Character classes use the `u` flag so non-ASCII matches at codepoint
// granularity (a single supplementary-plane character advances
// `lastIndex` by two UTF-16 code units in one step) and
// `\u{80}-\u{10FFFF}` reliably picks up any non-ASCII codepoint across
// every plane.
